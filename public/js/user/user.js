...
const openWindowCreateUser = document.querySelector(
  ".table-ticket .head-table .more-field button"
);
const closeWindowCreateUser = document.querySelector(
  ".window-create-user .painel .head-painel button"
);
  ...
  if (userData) {
    userData.map(
      ...
      <tr>
        ...
        <td>${h.nome}</td>
        <td>
          <button type="button" class="edit-user" value="${
            h.id
          }"><i class="fas fa-pen"></i></button>
            <button type="button" class="delete-user" value="${
              h.id
...
openWindowCreateUser.onclick = () => {
  ...
  boxNewPassword.nextElementSibling.querySelector("label").textContent =
    "Senha";
};
